...
function HN_CheckStartHordeNight()
	...
	-- It verify random hour option is enabled
	if SandboxVars.HordeNightMain.HordeNightRandomHour then

		hour = randomHour; -- If it's true, then horde night will start at a random day hour 
	else
		hour = SandboxVars.HordeNightMain.HordeNightHour; -- If don't, Horde Night will keel having its normal behavior.
	end
	...
	if daysPass >= SandboxVars.HordeNightMain.FirstHordeNightDay then
		...
		if dayAfterFirst % SandboxVars.HordeNightMain.HordeNightFrequency == 0 then
			...
			-- The horde is comming? 
			if getGameTime():getHour() % 24 == hour -1 then

				getSoundManager():PlaySound("SirenAlarm",false,0); -- Alert players!
			end

			-- The horde starts
			if getGameTime():getHour() % 24 == hour then
...
-- If the random start time is enabled, each day the start time will be random, so, on the day of the horde, they will have a random time to start
function HN_SetRandomHour()

	if SandboxVars.HordeNightMain.HordeNightRandomHour then
		
		randomHour = ZombRand(25);
		
		if randomHour <= getGameTime():getHour() % 24 then 

			randomHour = getGameTime():getHour() % 24;
			HN_CheckStartHordeNight();
		end
	end
end
...
Events.OnGameStart.Add(HN_SetRandomHour);
Events.EveryDays.Add(HN_SetRandomHour);
